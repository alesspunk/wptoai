...
function buildAiInstructions(validatedBundle, outputPlan) {
  ...
  return [
    ...
    "Prefer provided structure over newly invented layouts. If multiple sections exist, preserve them instead of merging them into a simpler composition.",
    "If navigation items exist in the approved inputs, include them in the output. Avoid reducing a page to a single hero section unless the provided context truly contains no other usable structure.",
    "If structuredData is limited, still use available pageStructure and sectionContent to generate simple but structured sections.",
    "Maintain structure fidelity without over-simplifying.",
    "",
    "PROJECT TREE GUIDELINE",
    "If a project tree is provided, use it as a guide for page creation, try to include all pages defined in the tree, and preserve page names when possible.",
    "If a page has limited data, generate a minimal placeholder version rather than omitting it completely unless no context exists at all.",
    "",
    ...
    "If structuredData.designSystem exists, you MUST use it to guide layout and styling decisions.",
    "Apply layoutPatterns by using headerPattern to determine header complexity, heroPattern to decide hero structure, gridPattern to build product/category grids, and sectionRhythm to organize vertical flow.",
    "Apply colorRoles by using primary and secondary for buttons and accents, background and surface for layout sections, and text and mutedText for typography hierarchy.",
    "Apply typographySystem by clearly separating heading versus body contrast and respecting headingScale when structuring sections.",
    "Apply spacingSystem by using sectionSpacing to define vertical rhythm and containerWidth for layout bounds.",
    "Apply shapeSystem by using borderRadius consistently and carrying buttonStyle and cardStyle across components.",
    "If structuredData.designSystem exists, do NOT fall back to generic layout generation. Instead, build layout using layoutPatterns and pageStructure.",
    "If structuredData.designSystem is incomplete, use available fields only and combine them with pageStructure and sectionContent.",
    "The output must feel like a reconstruction of the original system, not a generic template.",
    "",
    "COMPONENT PATTERN RULES",
    "If structuredData.componentPatterns exists, use it as a component-level reconstruction guide.",
    "Interpret and apply these patterns when available: header, hero, categoryStrip, productGrid, promoModules, and footer.",
    "Use componentPatterns to influence layout composition, section count, and section structure.",
    "Do not ignore clear component pattern signals when they exist.",
    "If componentPatterns is partial, use only the available patterns and combine them with pageStructure, designSystem, sectionContent, and screenshots.",
    "",
    "HEADER PATTERN GUIDELINE",
    "If componentPatterns.header exists, use header.pattern to guide header complexity.",
    "If the header pattern is multi-band, preserve multiple top regions when reasonable.",
    "If hasAnnouncementBar is true, allow a top promotional or utility strip.",
    "If hasUtilityNav is true, allow a secondary utility/account/search/cart region.",
    "If hasPrimaryNav is true, include a stronger primary navigation region.",
    "Use navItemCount only as a rough complexity hint, not a strict requirement, and use logoPosition as a visual hint when possible.",
    "Do not collapse a clearly multi-band header into a tiny generic header unless all other context strongly suggests a simpler structure.",
    "",
    "HERO PATTERN GUIDELINE",
    "If componentPatterns.hero exists, use hero.pattern to guide hero composition.",
    "Use contentAlignment to place copy more faithfully and use mediaPlacement to decide where the main visual belongs.",
    "If hasCTA is true, include a meaningful CTA.",
    "If hasOverlayText is true, allow copy over media when appropriate.",
    "Do not reduce a strong hero into a small generic text block if hero signals indicate a richer composition.",
    "",
    "CATEGORY / GRID / PROMO GUIDELINE",
    "If componentPatterns.categoryStrip exists and exists is true, preserve a distinct category strip or category-like repeated module, use itemCount as a rough hint, and use style as a visual hint only.",
    "If componentPatterns.productGrid exists and exists is true, preserve a distinct repeated card/grid section, use cardCountEstimate as a rough hint, and use cardStyle and columnPattern to shape the repeated layout.",
    "If componentPatterns.promoModules exists and exists is true, preserve one or more distinct promo sections, use moduleCountEstimate as a rough hint, and use style to prefer image-led versus copy-led promo composition.",
    "Do not collapse category, product, and promo structures into a single generic content block when the inputs indicate distinct repeated modules.",
    "",
    "FOOTER PATTERN GUIDELINE",
    "If componentPatterns.footer exists, use footer.pattern as a footer complexity hint.",
    "If hasNavLinks is true, include navigational footer links.",
    "If hasLegalText is true, include a simple legal or copyright area.",
    "If hasSocialLinks is true, include a social-links region when supported by available inputs.",
    "",
    "REAL HOMEPAGE ASSET RULES",
    "If structuredData.assets.realHomepageAssets exists, prefer those realHomepageAssets over generic placeholders and over screenshot-derived guesses.",
    "Use realHomepageAssets.logo, realHomepageAssets.heroImages, realHomepageAssets.categoryImages, and realHomepageAssets.promoImages as real reusable site assets when present.",
    "Interpret them as real source-site assets, not screenshots.",
    "If both old asset arrays and realHomepageAssets exist, prefer realHomepageAssets first and use the older arrays only as fallback.",
    "Do not overuse the same image repeatedly across unrelated sections.",
    "Use logo for header branding, heroImages for hero or hero-adjacent modules, categoryImages for category or tile sections, and promoImages for promo blocks when appropriate.",
    "",
    "ASSET FIDELITY GUIDELINE",
    "When real homepage assets exist, prefer them to make the output feel closer to the original site.",
    "Keep image placement aligned with component patterns and layout patterns.",
    "Do not scatter assets randomly, do not replace all imagery with one repeated visual, and do not fall back to generic neutral placeholders when real assets are available.",
    "",
    "SAFE FALLBACK RULES",
    "If componentPatterns or realHomepageAssets are missing or incomplete, use the available subset only.",
    "Combine them with pageStructure, sectionContent, designSystem, screenshots, and the approved page tree.",
    "Do not fail generation, and keep backward compatibility with simpler sites.",
    "These rules should improve fidelity without forcing complexity onto minimal websites.",
    "",
    "LAYOUT GENERATION GUIDELINE",
    "Avoid overly generic single-section layouts. Prefer a multi-section structure when the approved inputs or structured signals suggest one.",
    "Maintain separation between hero, content, and footer when possible.",
    "If unsure, choose a structured layout over a minimal layout.",
    "",
    "CONTENT RULES",
